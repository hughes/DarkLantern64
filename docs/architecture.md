...
These are the original inspection snapshots. [dependencies.json](../dependencies.json) pins the tested LightEngine editor integration revision. The ROM still uses a separately installed libdragon SDK; its actual compiler identity, headers, libraries, and packaging tools participate in the build fingerprint. Automated SDK provisioning remains work to do.

## Saved 3D implementation and integration boundary

Version-2 level JSON files directly inside `content/` are the canonical versioned scenes. The project editor manages these files; `content/level_bundle.json` selects the subset shipped in the game menu. It references reusable OBJ models and records stable IDs, XYZ positions, Euler rotations, scales, colors, collision proxies, and gameplay links. The compiler emits indexed XYZ meshes and instances for the runtime, plus glTF and scene JSON for LightEngine from the same model geometry. Runtime model rotations use radians; authoring rotations use degrees. Transforms apply scale, then X/Y/Z rotation, then translation. World units are meters in a shared right-handed XYZ system with +Y up. Authored yaw 0 faces +Z and yaw 180 faces -Z; right strafe and positive player turn follow the view's right side. The N64 camera's former horizontal mirror was corrected without moving geometry or changing collision coordinates.

The default renderer uses Tiny3D on the RSP for model transforms, rigid skeletal deformation and clipping, then the RDP for textured, depth-tested triangles. The CPU reference renderer remains available for comparisons. CPU code samples animation and calculates illumination; static faces cache illumination, while moving guards use two body-light probes to avoid hundreds of repeated visibility queries. The measured [guard renderer](guard-renderer-performance.md) and [depth correction](depth-precision.md) describe the shipped path and its limits.
...
| --- | --- |
| Larger-scene rendering and libdragon revision | Tiny3D is the default, with a tracked project precision patch applied in a disposable build stage. Measure larger content and original hardware before committing to final scene budgets. |
| Display mode and frame target | The two-guard workshop sustains every native refresh at 320x240 in Ares; [640x480 with AA](video-quality-study.md) sustains 30 fps there. Neither is a guarantee for larger missions. PAL timing and original hardware remain unverified. |
...
The [character animation prototype](guard-animation-prototype.md) implements a shared Blender-authored skeleton and five compressed clips, independent instance playback, connected rigid joints and bounded procedural head attention. LightEngine previews the same N64 pose sampler. General animation graphs, richer transitions and semantic foot-contact events remain future work.
