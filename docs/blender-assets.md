...
Run **DarkLantern64: Build + Play loot counter** to start directly in front of that counter. In the level selector, choose Moonlit Delivery Yard and use left/right to select **Blender loot counter**. The other rooms also offer a **Blender loot crate** start. Relaunch an older running editor to load the new import/placement controls.
...
1. In VS Code, run **DarkLantern64: Open loot in Blender**. Edit the meshes, UVs or atlas, then save the `.blend` file.
2. Run **DarkLantern64: Export Blender loot assets**. Blender runs in the background and writes `content/assets/loot/pack.json`, OBJ meshes and the texture image. This reads the saved source and does not change any level.
3. Open the desired level editor. In **Room Objects → Asset packs / props**, set **Pack URI** to `assets/loot/pack.json` and click **Import asset pack**. Choose the **Prefab**, then click **Place prop**. The manifest path is relative to `content/`.
...
5. **Save + Cook**, then run **DarkLantern64: Build + Play level select**, or the build/play task for the chosen level. Check the result in Ares under the level's actual lighting.
...
Run **DarkLantern64: Package Blender export add-on** to create `build/darklantern64_export.zip`. Install that ZIP through Blender's add-on preferences and enable **DarkLantern64 Asset Pack**. Packaging does not install it or change preferences automatically.
...
Use the exact prefab IDs listed in the exported `pack.json`. Target the correct running editor with `--level content/<level>.json`. Import and placement are editor changes; finish with `save` or the **Save + Cook** button.

**Recreate loot demo (overwrites loot.blend)** rebuilds the supplied example from its generator. It replaces that source and its exports, so save personal modeling changes to a different `.blend` file before choosing this task. Ordinary **Export Blender loot assets** preserves the source file.
